...
{
    [Collection("dapper")]
    public class MultiDBContextProviderTest : IDisposable
    {
        ...
        private readonly DapperRepository<SQLiteDapperTestEntity> _sqliteRepository;

        public MultiDBContextProviderTest()
        {
            ...
            _testContext = new DapperContext(new DapperRuntime(optionsMock.Object,
                    new IDapperMetadataProvider[] {
                        new MySQLDapperMetadataProviderInstance(),
                        new SQLiteDapperMetadataProviderInstance()
                    })
                , new LoggerFactory());
            _mySQLRepository = new DapperRepository<DapperTestEntity>(_testContext);
            Assert.NotNull(_mySQLRepository);
            _sqliteRepository = new DapperRepository<SQLiteDapperTestEntity>(_testContext);
        ...
        private SQLiteDapperTestEntity CreateSQLiteDapperTestEntity(int id = 1)
        {
...
,{entity.int_value},{entity.mediumint_value},{entity.numeric_value},{entity.real_value},{entity.smallint_value},{entity.tinyint_value},'{entity.longtext_null_value}')");
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:连接名对比")]
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:查询首个或者默认值")]
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:异步查询首个或者默认值")]
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:查询所有操作")]
        private void QueryAll()
        {
            ...
            }
            {
                ...
                try
                {
                    ...
                    var result = _sqliteRepository.QueryAll() ?? new List<SQLiteDapperTestEntity>();
                    var sql = "select * from dapper_all_test";
                    SQLiteHelper.ExecuteReader(sql, reader =>
                    {
                        while (reader.Read())
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:异步查询所有操作")]
        private void QueryAllAsync()
        {
            ...
            }
            {
                ...
                try
                {
                    var result = _sqliteRepository.QueryAllAsync().GetAwaiter().GetResult() ?? new List<SQLiteDapperTestEntity>();
                    var sql = "select * from dapper_all_test";
                    SQLiteHelper.ExecuteReader(sql, reader =>
                    {
                        while (reader.Read())
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:查询操作")]
        private void Query()
        {
            {
                ClearMySQLDapperTestEntity();
            ...
            }
            {
                ...
                try
                {
                    var filter = new SingleQueryFilter().AddEqual(nameof(SQLiteDapperTestEntity.id), 2);
                    var result = _sqliteRepository.Query(filter);
                    SQLiteHelper.ExecuteReader(@"select * from ""dapper_all_test"" where ""id""=2", reader =>
                    {
                        while (reader.Read())
                        {
                            CompareHelper.Compare(reader, result.FirstOrDefault(x => x.id == Convert.ToInt32(reader["id"])));
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:异步查询操作")]
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:插入操作")]
        public void Insert()
        {
            {
                #region 实体对象赋值
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:异步插入操作")]
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:批量插入操作")]
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:删除操作")]
        public void Delete()
        {
            {
                ClearMySQLDapperTestEntity();
            ...
            }
            {
                ...
                {
                    var d1 = CreateSQLiteDapperTestEntity();
                    _sqliteRepository.Delete(new SQLiteDapperTestEntity { id = d1.id });
                    var sql = $"select count(*) from dapper_all_test where id = {d1.id}";
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:异步删除操作")]
        public void DeleteSync()
        {
            ...
            }
            {
                ...
                {
                    var d1 = CreateSQLiteDapperTestEntity();
                    _sqliteRepository.DeleteAsync(new SQLiteDapperTestEntity { id = d1.id }).GetAwaiter().GetResult();
                    var sql = $"select count(*) from dapper_all_test where id = {d1.id}";
                    var count = SQLiteHelper.GetValue(sql, Convert.ToInt32);
                    Assert.Equal(count, 0);
                }
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:更新操作")]
        public void Update()
        {
            {
                ClearMySQLDapperTestEntity();
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:异步更新操作")]
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:分页查询")]
        ...
        [Fact(DisplayName = "MultiDBContext.Provider:异步分页查询")]
        ...
        private class SQLiteDapperMetadataProviderInstance : DapperMetadataProvider<SQLiteDapperTestEntity>
        {
            protected override void ConfigureModel(DapperMetadataBuilder<SQLiteDapperTestEntity> builder)
            {
                builder.HasKey(d => d.id).TableName("dapper_all_test").DataSource(SQLITE);
            }
        }
