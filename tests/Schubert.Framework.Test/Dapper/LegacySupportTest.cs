...
{
    [Collection("dapper")]
    public class LegacySupportTest : IDisposable
    {
        ...
        private void ClearTestEntity() => helper.ExecuteSQL("delete from DapperAllTest", command => command.ExecuteNonQuery());
        ...
        public void Update()
        {
            ...
            try
            {
                ...
                _dapper.Update(d1);
                string sql = string.Format("select * from DapperAllTest where id = {0}", d1.Id);
                helper.ExecuteReader(sql, reader =>
                {
                    while (reader.Read())
                    {
                        CompareHelper.ComparePascalCase(reader, d1);
                    }
                });
            }
            finally
