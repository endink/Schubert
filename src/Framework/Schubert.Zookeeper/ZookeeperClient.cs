...
{
    /// <summary>
    /// 表示一个 ZookeeperClient。
    /// </summary>
    public class ZookeeperClient : Watcher, IDisposable
    {
        #region Field
        ...
        public ZookeeperClient(string connectionString)
            : this(new ZookeeperClientSettings(connectionString))
        {
        }
        ...
        /// 客户端选项。
        /// </summary>
        public ZookeeperClientSettings Options { get; }
        ...
        public async Task<T> RetryUntilConnected<T>(Func<Task<T>> callable)
        {
            ...
            while (true)
            {
                ...
                if (DateTime.Now - operationStartTime > TimeSpan.FromSeconds(Options.OperatingTimeoutSeconds))
                {
                    throw new TimeoutException($"Operation cannot be retried because of retry timeout ({Options.OperatingTimeoutSeconds * 1000} milli seconds)");
                }
            }
        }
        ...
        private ZooKeeper CreateZooKeeper()
        {
            ...
            {
                return new ZooKeeper(Options.ConnectionString, (int)Options.SessionTimeoutSeconds * 1000, this, Options.SessionId, Options.SessionPasswd, Options.ReadOnly);
            }
            else
            {
                return new ZooKeeper(Options.ConnectionString, (int)Options.SessionTimeoutSeconds * 1000, this, Options.ReadOnly);
            }
        }

        private async Task ReConnect()
        {
            if (!Monitor.TryEnter(_zkEventLock, Options.ConnectionTimeoutSeconds))
                return;
            try
            {
                if (ZooKeeper != null)
        ...
        {
            var basePath = (Options.BasePath == null || String.IsNullOrWhiteSpace(Options.BasePath)) ? "/" : Options.BasePath.Trim();
